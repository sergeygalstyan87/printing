$(document).ready(function () {
    ...
    $(document).on('click', '.add_delivery_type', function () {
        ...
        $('.delivery_groups').append('<div class="form-group delivery_group"><div class="col-3"><label>Title</label> <input type="text" class="form-control" placeholder="Delivery title" name="delivery[' + deliveries + '][title]" value="" required> </div> ' +
            '<div class="col-3"> <label>Percent</label> <input type="number" class="form-control" placeholder="Delivery percent" name="delivery[' + deliveries + '][price]" value="0" required></div>' +
            '<div class="col-3"> <label>Days</label> <input type="number" class="form-control" placeholder="Delivery days" name="delivery[' + deliveries + '][days]" value="1" min="1" max="365" step="1" required></div>' +
            '<div class="is_over_time"><label>Is Over</label><input class="form-control is_over_checkbox" type="checkbox" name="delivery[' + deliveries + '][is_over]"> </div>' +
            '<div class="is_over_count d-none"><label>Over count</label><input type="number" class="form-control" name="delivery[' + deliveries + '][is_over_count]"/></div>'+
        '<button type="button" class="btn btn-danger delete_delivery_type">Delete</button></div>'
    )
        ;
        deliveries++
    })

    $(document).on('change', '.is_over_checkbox', function (){
        const isChecked = $(this).is(':checked');

        // Hide all is_over_count blocks
        $('.is_over_count').addClass('d-none');

        // Empty the value of all is_over_count inputs
        $('.is_over_count input[type="number"]').val('');

        // Uncheck all other checkboxes
        $('.is_over_checkbox').not(this).prop('checked', false);

        // Show associated is_over_count block if checkbox is checked
        if (isChecked) {
            $(this).closest('.is_over_time').next('.is_over_count').removeClass('d-none');
        }
    });
    ...
    $(document).on('change', '#subcategory_id', function () {
        ...
        let cat_id = $(this).val();
        let categories = categories_array.filter(function (el) {
            return el.id == cat_id;
        });

        if (categories.length > 0) {
            $('#design_price').val(categories[0].design_price);
        }
    });
    ...
    function generateQuantities(quantity) {
        ...
        if (quantities_blocks.find('.quantity_' + quantity).length === 0) {
            quantities_blocks.append('<div class="quantity_block quantity_' + quantity + '" data-quantity="' + quantity + '">' +
                ...
                '<h4>Discount for quantity ' + quantities_arr[quantity] + '</h4>' +
                '' +
                '</div>' +
                '<div class="quantity_attributes">' +
                '<input type="number" name="quantity_discount['+quantity+'][discount]" value=""/>' +
                '<input type="hidden" name="quantity_discount['+quantity+'][value]" value="'+ quantities_arr[quantity] +'"/>' +
                '</div>' +
        ...
        attributes.on("select2:selecting", function (e) {
            ...
            if( check > -1){
                ...
                $('.attribute_types').append('<div class="form-group row type_group type_' + attribute_id + '">' +
                    ...
                    '<select id="attribute_' + attribute_id + '" multiple="multiple" class="form-control" name="types['+attribute_id+'][]" >' + options + '</select>' +
                ...
                attr.on("select2:selecting", function (e) {
                    ...
                    var new_attrs = {};
                    if(typeof related_types[type_id] == 'object'){
                        var new_attrs = related_types[type_id];
                    }
                    $.each(new_attrs, function (index, new_a) {
                        var options = attributes.find('option');
                        for (var i = 0; i < options.length; i++) {
                            var option = $(options[i]);
                            if (option.data('id') == new_a) {
                                var ttt =  option.data('title');
                            }
                        }
                        attributes.trigger({
                                ...
                                params: {
                                    args: {
                                        data: {
                                            id: new_a, // The option value you want to select
                                            text: ttt, // The option text
                                        },
                                    },
                                },
                            });
                ...
                attr.on('select2:unselect', function (e) {
                    ...
                    if(typeof related_types[elem.id] == 'object'){
                        var new_attrs = related_types[elem.id];
                    }
                    $.each(new_attrs, function (index, new_a) {

                        var optionToUnselect = attributes.find('option[data-id="' + new_a + '"]');
