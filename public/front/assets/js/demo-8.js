...
(function($) {
    ...
    $(document).ready(function(){
        ...
        $(".ec-sidebar-block .ec-sidebar-block-item").not($(this)).removeClass('open');
        $(this).toggleClass('open');
        var $this = $(this).closest('.ec-sb-block-content').find('.ec-cat-sub-dropdown');
        $this.slideToggle('slow');
        $('.ec-cat-sub-dropdown').not($this).slideUp('slow');
    });
        $(".ec-sidebar-block .ec-sidebar-sub-item").on("click", function() {
            $(".ec-sidebar-block .ec-sidebar-sub-item").not($(this)).removeClass('open');
            $(this).toggleClass('open');
            var $this = $(this).next('.ec-prod-sub-dropdown');
            $this.slideToggle('slow');
            $('.ec-prod-sub-dropdown').not($this).slideUp('slow');
        });
    });
    ...
    function addClassBasedOnDevice() {
        const isMobile = window.screen.width <= 430;
        const isTablet = window.screen.width > 430 && window.screen.width <= 1024;
        const bannerBlock = $('.home_banner_block');
        let imageUrl = bannerBlock.data('default-image');

        let mobile = $('.main_slider_mobile');
        let tablet = $('.main_slider_tablet');
        let elementWebSlide = $('.ec-slider.swiper-container');
        if (isMobile) {
            mobile.addClass('ec-slider swiper-container').removeClass('hidden');
            elementWebSlide.addClass('hidden').removeClass('ec-slider swiper-container');
            if(bannerBlock.data('mobile-image')){
                imageUrl = bannerBlock.data('mobile-image');
            }
        }else if (isTablet){
            tablet.addClass('ec-slider swiper-container').removeClass('hidden');
            elementWebSlide.addClass('hidden').removeClass('ec-slider swiper-container');
            if(bannerBlock.data('tablet-image')){
                imageUrl = bannerBlock.data('tablet-image');
            }
        }

        // Update background image of .ec-banner-block
        bannerBlock.css('background-image', 'url(' + imageUrl + ')');
    }

    addClassBasedOnDevice();
    ...
    // Check if there's a slider with the category_slider class
    if (document.querySelectorAll('.category_slider .ec-slider.swiper-container .swiper-wrapper').length > 0) {
        var uniqueSlides = document.querySelectorAll('.category_slider .ec-slider.swiper-container .swiper-wrapper').length;
        if (uniqueSlides === 1) {
            loopEnabled = false;
        }
    }

    // Initialize Swiper with the appropriate loop option
    var EcMainSlider = new Swiper('.ec-slider.swiper-container', {
        loop: loopEnabled,
        watchOverflow: true, // Enable this to handle single-slide case
        speed: 1000,
        effect: "slide",
        autoplay: {
            delay: 7000,
...
/*--------------------- Trend Product Slider -------------------------------- */
    ...
    $('#partners_logo-slider').owlCarousel({
        loop: true,
        dots:false,
        nav:false,
        smartSpeed: 1000,
