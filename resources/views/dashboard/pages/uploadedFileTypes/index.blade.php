...
@section('content')
    ...
    <section class="page-content container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <h5 class="card-header">Uploaded file types</h5>
                    <div class="card-body">
                            ...
                            @if( !empty($items) )
                                @foreach($items as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->title }}</td>
                                        <td>
                                            <a href="{{ route('dashboard.uploadedFileTypes.edit', $item->id) }}"
                                               class="btn btn-sm btn-light btn-active-light-danger"
                                               style="display: inline-flex;align-items: center;">Edit</a>
                                            @if($item->id !== 1)
                                                <form action="{{ route('dashboard.uploadedFileTypes.delete', $item->id) }}"
