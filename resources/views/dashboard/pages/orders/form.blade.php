                    ...
                    <div class="tab-pane fade show" id="customer_info" role="tabpanel"
                        ...
                        <div class="row mb-2">
                            <div class="col-6">
                                ...
                                <div class="font-weight-500"><b>{{ (!empty($item->name)?$item->name:'N/A') }}</b></div>
                            </div>
                            <div class="col-6">
                            ...
                            <div class="col-6">
                                @if( isset($item) && $item->images )
                                    @foreach($item->images as $image)
                                        <div class="mr-3">
                                            <embed src="https://drive.google.com/file/d/{{$image}}/preview" width="180" height="200" allow="autoplay">
                                        </div>
                                    @endforeach
                                @endif
                            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="terminalModalLabel">Terminal Payment</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <img src="{{ asset('admin/assets/img/payment-icons/image_processing.gif') }}" alt="Terminal Image" class="img-fluid">
                    <p>Your terminal payment is being processed...</p>
                </div>
                <div class="modal-footer">
                    <button type="button" id="cancel-payment-btn" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                </div>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <!-- Trigger the modal if the flag exists -->
    @if(session('show_terminal_modal'))
        <script>
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if(response.status === 'completed' || response.status === 'canceled'){
                                    ...
                                    if (response.flash.level === 'success') {
                                        toastr.success(response.flash.message);
                                    } else if (response.flash.level === 'error') {
                                        toastr.error(response.flash.message);
                                    }
                                }
                            },
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#cancel-payment-btn').on('click', function() {
                $.ajax({
                    type: 'POST',
                    url: '{{ route("dashboard.orders.cancel_terminal_payment") }}',
                    data: {
                        _token: '{{ csrf_token() }}',
