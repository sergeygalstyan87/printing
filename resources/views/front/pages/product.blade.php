...
@section('content')
    ...
    <form id="calculation_form" class="section-space-pt" action="{{$project ? route('basket.updateItem', $project) : route('basket.addItem')}}" method="post" enctype="multipart/form-data">
                                    ...
                                    <div class="single-pro-desc single-pro-desc-no-sidebar">
                                        <div class="single-pro-content">
                                            ...
                                            <div class="first_step">
                                                @include('front.pages.partials.product.steps.first_step')
                                            </div>
                                            <div class="second_step">
                                                @include('front.pages.partials.product.steps.second_step')
                                            </div>
                                            ...
                                            <div class="ec-single-qty">
                                                <div class="ec-single-cart ">
                                                    <button class="btn btn-primary product_continue"
                                                            data-id="{{ $product->id }}">
                                                    <span class="continue_with_icon">
                                                        <span>Continue</span>
{{--                                                        <img class="right-icon"--}}
{{--                                                             src="{{ asset('/front/assets/images/right_arrow.svg') }}"--}}
{{--                                                             alt="Get Started" width="35">--}}
                                                    </span>
                                                    </button>

                                                    <div class="mobile_total_block">
                                                        <div class="price_container">
                                                            <span class="price_container_text">Printing Cost:</span>
                                                            <div class="new-price price_data"></div>
                                                        </div>
                                                    ...
                                                    <div class="upload_files_block hidden">
                                                        <div class="upload_files_types">
                                                            <div class="upload_files_types_item">
                                                                <input id="order_type_1" type="radio" name="type"
                                                                       value="Upload Print Ready Files" {{$project && ($project->type == 'Upload Print Ready Files') ? 'checked' : ''}}
                                                                       data-set-id="{{$project ? json_encode($project->sets->pluck('id')) : ''}}"
                                                                       data-type-ids="{{$project ? json_encode($project->attrs['selected_values']) : ''}}"
                                                                />
                                                                <label for="order_type_1" class="upload_files_type upload_type_1 upload_now_btn" data-toggle="modal" data-target="#uploadModal">
                                                                    <span class="upload_type_text">Upload Print Ready Files</span>
                                                                </label>
                                                                <input id="order_type_2" type="radio" name="type" value="Order Now Upload Later" {{$project && ($project->type == 'Order Now Upload Later') ? 'checked' : ''}}/>
                                                                <label for="order_type_2" class="upload_files_type upload_type_2">
                                                                    <span class="upload_type_text">Order Now Upload Later</span>
                                                                </label>
                                                            </div>
                                                            <div class="upload_files_types_item">
                                                                <input id="order_type_3" type="radio" name="type" value="Design Offer" data-price="{{$product->design_price ?? 0}}" {{$project && ($project->type == 'Design Offer') ? 'checked' : ''}}/>
                                                                <label for="order_type_3" class="upload_files_type upload_type_3">
                                                                    <span class="upload_type_text">Purchase a Design Service (${{$product->design_price}})</span>
                                                                </label>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="buttons_block hidden">
                                                        <div class="buttons_block_list">
                                                            <button class="btn btn-primary back_first_step">
                                                                Back
                                                            </button>
                                                            @if($project)
                                                                <button class="btn btn-primary update_cart"
                                                                        data-id="{{ $product->id }}">
                                                                    Update Cart
                                                                </button>
                                                            @else
                                                                <button class="btn btn-primary add_cart"
                                                                        data-id="{{ $product->id }}">
                                                                    Add to cart
                                                                </button>
                                                            @endif
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('.opened_list_item').first().addClass('selectric-selected');

            $('.opened_list_item').on('click', function () {
                $('.opened_list_item').removeClass('selectric-selected');
                $(this).addClass('selectric-selected');
                const typeId = $(this).data('id');
                const aaaId = $(this).data('attr');

                $('#attribute_' + aaaId).val(typeId);
                $('#attribute_' + aaaId).trigger('change');
            });
