            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <h2 class="checkout_heading">
                        <span class="checkout_step checkout_step_4">2</span>
                        Payment
                ...
                <div class="col-md-4">
                    ...
                    <table class="checkout_right w-100 mt-3" style="line-height:30px">
                        ...
                        <tr>
                            <td>Shipping</td>
                            <td class="shipping_cost">${{ $order->shipping_price }}</td>
                        </tr>
                        ...
                        <tr style="border-top: 1px solid">
                            ...
                            <td>Tax</td>
                            <td>$<span  id="tax_amount">{{$order->tax}}</span></td>
                        </tr>
                        <tr style="border-top: 1px solid;font-size: 20px;font-weight: bold;">
                            <td>Total</td>
                            <td class="total_cost">${{ number_format($order->amount, 2, '.', '') }}</td>
                        </tr>
                    </table>
                </div>
            </div>
...
k++;
                ...
                $(document).on('click', '.delete_img', function () {
                    $(this).parents('.upload_image_preview').remove()
                })
                ...
                $(document).on('submit', '#place_order_form', function (e) {
                    ...
                    $("#ec-overlay").show();
                    var type_id = 'upload_type_2_form';

                    var type_i = $("input[name='type']:checked").val();
                    if (type_i === 'Upload Print Ready Files') {
                      var type_id = 'upload_type_1_form';
                    } else if (type_i === 'Order Now Upload Later') {
                        type_id = 'upload_type_3_form';
                    } else {
                        type_id = 'upload_type_2_form';
                    }
                    const formElement = document.getElementById(type_id);


                    let typesForm = new FormData(formElement);
                    ...
                    mergeFormData(mergedFormData, formData);
                    mergeFormData(mergedFormData, typesForm);
                ...
                function mergeFormData(targetFormData, sourceFormData) {
                    // Iterate through each key-value pair in the source FormData
                    for (const [key, value] of sourceFormData.entries()) {
                        // Append the key-value pair to the target FormData
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.status) {
                                ...
                                <p><b>Unit:</b>  ${data.address.unit}</p>
